...
import { mockContractorMissions } from "../data/mockData";
import { CheckCircle2 } from "lucide-react";

export function ContractorMissionsTab() {
  const missions = mockContractorMissions;
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2 pt-2">
              <Button className="flex-1" size="sm">
                Proposer honoraires ({mission.price})
              </Button>
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="mb-4">
        <h2 className="text-2xl font-bold text-foreground">Gestion des Chantiers</h2>
        <p className="text-muted-foreground">Suivez vos projets de rénovation.</p>
      </div>
      ...
      <Tabs defaultValue="available" className="w-full">
        ...
        <TabsContent value="available" className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {available.length === 0 && <p className="text-center text-muted-foreground col-span-2 py-8">Aucun chantier disponible.</p>}
        </TabsContent>
        <TabsContent value="in_progress" className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {inProgress.length === 0 && <p className="text-center text-muted-foreground col-span-2 py-8">Aucun chantier en cours.</p>}
        </TabsContent>
        <TabsContent value="finished" className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {finished.length === 0 && <p className="text-center text-muted-foreground col-span-2 py-8">Aucun chantier terminé.</p>}
        </TabsContent>
        <TabsContent value="refused" className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          {refused.length === 0 && <p className="text-center text-muted-foreground col-span-2 py-8">Aucun chantier refusé.</p>}
        </TabsContent>
      </Tabs>
    </div>
  );
