...
import { mockDiagnosticianQuotes } from "../data/mockData";
import { FileText, Link as LinkIcon } from "lucide-react";

export function DiagnosticianQuotesTab() {
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Vos Devis Diagnostics</h2>
      ...
      <div className="grid grid-cols-1 gap-4">
        {mockDiagnosticianQuotes.map((quote) => (
          <Card key={quote.id} className="border-border bg-card/80 backdrop-blur-sm">
            <CardContent className="p-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <Badge variant={quote.status === "Envoyé" ? "secondary" : "default"}>{quote.status}</Badge>
