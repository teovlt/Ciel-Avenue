...
import { ChevronLeft, ChevronRight, MapPin, Clock, Calendar as CalendarIcon, ClipboardCheck, AlertTriangle, User } from "lucide-react";
import { BookingActions } from "../shared/BookingActions";
// Mock data around appointments
const mockAppointments = [
  { id: 1, date: 15, title: "Diagnostic DPE + Amiante", client: "M. Martin", time: "09:00", type: "dpe", location: "Aubagne, 13400" },
  { id: 2, date: 15, title: "Diagnostic Plomb", client: "SCI Les Oliviers", time: "14:00", type: "plomb", location: "Marseille, 13008" },
  {
    id: 3,
    date: 18,
    title: "Pack Vente Maison",
    client: "Agence Immobilière Sud",
    time: "10:30",
    type: "pack",
    location: "Aix-en-Provence, 13100",
  },
  { id: 4, date: 22, title: "État des Risques (ERP)", client: "Mme Dubois", time: "16:00", type: "erp", location: "Cassis, 13260" },
  { id: 5, date: 28, title: "Diagnostic Électricité", client: "M. Lemaire", time: "09:30", type: "elec", location: "Marseille, 13012" },
];

export function DiagnosticianAgendaTab() {
  ...
  const getIcon = (type: string) => {
    if (type === "erp") return <AlertTriangle className="h-3 w-3" />;
    if (type === "pack") return <ClipboardCheck className="h-3 w-3" />;
    ...
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-muted-foreground">Planifiez vos visites et vos diagnostics immobiliers.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <h3 className="font-bold text-lg px-1 flex items-center gap-2">
            <ClipboardCheck className="h-5 w-5 text-primary" /> Vos visites
          </h3>
                  ...
                  <CardContent className="p-4 space-y-3">
                    <div className="flex justify-between items-start">
                      <Badge variant="outline" className={`${getBadgeColor(app.type)} flex items-center gap-1.5 px-2 py-0.5`}>
                        {getIcon(app.type)}
                        <span className="capitalize">{app.type.toUpperCase()}</span>
                      </Badge>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-primary font-medium mt-1 flex items-center gap-1">
                        <User className="h-3 w-3" /> {app.client}
                      </p>
                    </div>
