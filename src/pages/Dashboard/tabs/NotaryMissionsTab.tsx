...
import { mockNotaryMissionsExtended } from "../data/mockData";
import { CheckCircle2 } from "lucide-react";

export function NotaryMissionsTab() {
  const missions = mockNotaryMissionsExtended;
      ...
      <CardContent className="p-6 space-y-4">
          ...
          <div className="space-y-2 bg-muted/50 p-3 rounded-lg">
            ...
            {mission.rating && (
              <div className="text-sm">
                Note client: <span className="font-bold">{mission.rating}/5</span>
              </div>
            )}
        ...
        {type === "refused" && (
          <div className="text-sm text-muted-foreground">
            <p>Refusé le {mission.date}</p>
            <p>Raison: {mission.reason}</p>
          </div>
        )}
      </CardContent>
    </Card>
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="mb-4">
        <h2 className="text-2xl font-bold text-foreground">Gestion des Missions</h2>
        <p className="text-muted-foreground">Suivez vos dossiers par étape.</p>
      </div>
      ...
      <Tabs defaultValue="available" className="w-full">
        ...
        <TabsContent value="refused" className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          {refused.map((m) => renderMissionCard(m, "refused"))}
