...
import { mockNotaryMissionsOverview } from "../data/mockData";
import { ArrowRight } from "lucide-react";
import { Button } from "@/components/ui/button";

export function NotaryOverviewTab() {
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-foreground">Types de missions disponibles</h2>
        <p className="text-muted-foreground">6 types de prestations notariales pour accompagner tous les besoins immobiliers.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mockNotaryMissionsOverview.map((item) => (
          <Card key={item.id} className="border-border bg-card/80 backdrop-blur-sm card-hover-lift cursor-pointer group">
            <CardContent className="p-6 space-y-4">
              ...
              <Button variant="ghost" className="w-full justify-between p-0 hover:bg-transparent text-foreground group-hover:text-primary">
