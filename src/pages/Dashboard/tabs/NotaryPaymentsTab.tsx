...
import { mockNotaryPayments } from "../data/mockData";
import { Euro, ArrowUpRight, Clock } from "lucide-react";

export function NotaryPaymentsTab() {
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-foreground">Paiements & Facturation</h2>
        <p className="text-muted-foreground">Suivi de vos revenus et encaissements.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">2 factures à encaisser</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">15 450,00 €</div>
            <p className="text-xs text-muted-foreground">42 missions réalisées</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-border bg-card/80 backdrop-blur-sm">
        ...
        <CardContent>
          <div className="space-y-4">
            {mockNotaryPayments.map((payment) => (
              <div
                key={payment.id}
